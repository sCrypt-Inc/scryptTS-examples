...
async function recoverAfterDeployed() {
    ...
    const meInstance = new Recallable(PubKey(toHex(myPublicKey)))
    // sync state from tx
    meInstance.syncState(deployTx, atOutputIndex)
    // connect a signer
...
async function recoverAfterCalled() {
    ...
    // create an instance with the data when deploying the contract
    const aliceInstance = new Recallable(PubKey(toHex(myPublicKey)))
    // sync state from tx
    aliceInstance.syncState(lastCallTx, atOutputIndex)
    // connect a signer
